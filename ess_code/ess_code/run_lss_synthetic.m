function run_lss_synthetic()
% Run the line slice sampler on the synthetic data.
...
for num_dims = 1:setup.max_dim
    name = sprintf('lss_synthetic_%dd', num_dims);
    fn = @(run) synthetic_run(setup, num_dims, run);
    success = experiment_run(name, setup.runs, fn, true);
end


function lss_results = synthetic_run(setup, num_dims, run)
...
for ii = (1-burn):iterations
  ...
  [ff, cur_log_like] = gppu_lineslice(ff, data{num_dims}.chol_cov, counting_log_like_fn, cur_log_like);
...
fprintf('Run %03d/%3d %02d-d Effective Samples: %0.2f  %0.2f secs\n\n', ...
  run, runs, num_dims, lss_results.effective_samples, elapsed);
